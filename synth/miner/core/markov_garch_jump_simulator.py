...
def simulate_markov_garch_jump(
    prices_dict: dict,
    asset: str,
    time_increment: int,
    time_length: int,
    ...
    **params
    ...
    if asset in ASSET_PRIORS:
        for k, v in ASSET_PRIORS[asset].items():
    ...
    if asset.upper() in {"BTC", "ETH"}:
        params["lookback_days"] = params.get("lookback_days", 15)
