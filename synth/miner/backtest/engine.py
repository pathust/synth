...
class BacktestEngine:
    ...
    def __init__(
        self,
        metric: str = "CRPS",
        data_loader: Optional[UnifiedDataLoader] = None,
    ...
    def _get_prompt_config(self, asset: str):
    ...
    def run_slots(
        ...
        results = []
        cfg = self._get_prompt_config(asset)
        for slot in eval_slots:
            ...
            if paths is None or len(paths) == 0:
                continue
            truth = self.data_loader.get_future_data(
                asset,
                start_time=slot,
                time_length=cfg.time_length,
            )
            ...
            results.append({"slot": slot, "metrics": slot_metrics})
        return results
