...
from synth.miner.data.storage import sync_price_dict_to_duckdb
...
MAX_WORKERS = 4
DUCKDB_MIRROR_ENABLED = os.getenv("SYNTH_ENABLE_DUCKDB_WRITER", "1") == "1"
    ...
    try:
        ...
        if latest_ts is None:
            ...
            if staleness >= time_increment:
                ...
                status = "OK (incremental)"
            else:
                status = "OK (up-to-date)"
        _mirror_asset_to_duckdb(asset, time_frame)
        return (asset, True, status)
    except Exception as e:
        return (asset, False, str(e))


def _mirror_asset_to_duckdb(asset: str, time_frame: str) -> None:
    if not DUCKDB_MIRROR_ENABLED:
        return
    data = data_handler.load_price_data(asset, time_frame)
    if not data or time_frame not in data:
        return
    prices = data[time_frame]
    if not prices:
        return
    sync_price_dict_to_duckdb(
        symbol=asset,
        prices=prices,
        source=f"mysql_{time_frame}",
        max_rows=500,
    )
